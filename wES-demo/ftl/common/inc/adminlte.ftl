...
<!--${adminpage_activeMenu.id}     ==>    ${td.id} ==> ${active?string}-->
	...
	<li class="${active?string('active ', '')} treeview">
		<a href="#">
			<i class="fa ${td.icon!'fa-circle-o'}"></i> <span>${td.name}</span> <i
	...
	<#else>
	<!-- ${td.id} -->
	<li class="${active?string('active ', '')}"><a
			href="${((td.target!'') =='mainFrame')?string('/a/toIndex?menuId='+td.id, td.href!'')}"><i
			class="fa ${td.icon!'fa-circle-o'}"></i> <span>${td.name!''}</span></a></li>
	</#if>
</#macro>
...
<ol class="breadcrumb">
	...
	<#if (adminpage_activeMenu.parent.name)??>
		<li
		<a href="${(adminpage_activeMenu.parent.href)!'#'}"
		   target="_blank">${(adminpage_activeMenu.parent.name)!''}</a></li>
	</#if>
		...
		<div class="col-xs-${(body?trim?length>0)?string('8','8')} col-sm-${(body?trim?length>0)?string('6','9')}">
			...
			<#elseif type=='readonly'>
				<input type="text" class="form-control" id="${id}" name="${name}" placeholder="${placeholder}"
				       readonly="readonly" value="${value}" ${code}>
				<#if body?trim?length gt 0>
				</div>
				<div class="col-xs-8 col-sm-3 control-label-right">${body}</#if>
			<#elseif type=='textarea'>
				<textarea id="${id}" class="form-control" name="${name}"
				          placeholder="${placeholder}" ${code}>${value}</textarea>
			<#else>
				<#if body?trim?length gt 0>
				</div>
				...
				<#list files as file>
					<tr>
						...
						<td style="text-align: center;width:60px;"><a
								href="/a/file/files/downloadFile?id=${file.id}">下载</a></td>
					</tr>
				</#list>
			</table>
		</div>
...
<div class="${a.autoCol(12)}">
	<div class="form-group">
		...
		<@LoadJs>
			...
			$('#${id}').click(function() {
			fu_${id}.setParams({groupId:'${groupId}', isCut:${(cutSize!='')?string('true', 'false')},
			cutSize:'${cutSize}'});
			...
			<#if files?size gt 0>
				var fu_${id}Files = [];
				<#list files as file>
					fu_${id}Files.push({uuid:"${file.id}", name:"${file.filename}", size:"",
					thumbnailUrl:"${file.filepath}", status: qq.status.UPLOAD_SUCCESSFUL});
				</#list>
...
<#macro TitleRow title subTitle=''>
	...
	<div class="row">
		<div class="col-xs-12">
			...
			${title}
				<#if subTitle != ''>
					<small class="pull-right">${subTitle}</small></#if>
			</h2>
		</div>
		<!-- /.col -->
	</div>
...
<#macro TextBtn text id=''cls='default' icon='' ext='' >
	<button <#if id!=''> id="${id}" name="${id}" </#if> type="button" class="btn btn-${cls}" ${ext}><#if icon!=''><i
			class="fa ${icon} hidden-xs"></i></#if> ${text} </button>
</#macro>
...
<#macro pagination page formId='searchForm'>
	...
	<@LoadJs>
		function page(n,s){
		$("#${formId}").append("<input id='pageNo' name='pageNo' type='hidden' value='${page.pageNo}'/><input
			id='page.size' name='pageSize' type='hidden' value='${page.pageSize}'/>");
		$("#pageNo").val(n);
		$("#pageSize").val(s);
		$("#${formId}").submit();
		return false;
	...
	<div class="${autoCol(col)}">
		<div class="form-group">
			...
			<div class="col-xs-8 col-sm-9">
				<select id="${id}" name="${id}" class="form-control select2" data-placeholder="请选择${label}"
				        style="width: 100%;"  ${ext}>
					<#nested>
				</select>
			</div>
		...
		<div class="form-group">
			...
			<div class="col-xs-8 col-sm-9">
				<div class="input-group">
					<input type="text" id="${id}" name="${id}" class="form-control pull-right" ${ext} readonly><span
						class="input-group-addon" onclick="datetime_remove_${id}();"><i class="fa fa-remove"></i></span>
					<@LoadJs>
						$('#${id}').datetimepicker({
						language: "zh-CN",
						autoclose:true,
		...
		<div class="form-group">
			...
			<div class="col-xs-8 col-sm-9">
				<div class="input-group" style="width:100%">
					<button type="button" class="btn btn-default pull-right" id="${id}"
					        style="width:100%;text-align:left;background-color:#fff;border-radius:0">
						<i class="fa fa-calendar"
						   style="margin-right:16px;"></i><span><#if timeStart!=''&&timeEnd!=''>${timeStart}
						- ${timeEnd}</#if></span>
					</button>
					<input type="hidden" id="${id}_hidden_time_start" name="timeStart" value="${timeStart}"/>
					<input type="hidden" id="${id}_hidden_time_end" name="timeEnd" value="${timeEnd}"/>
...
<#macro modalButton target id="" cls='default' icon=''  ext="">
	<button <#if id!=''> id="${id}" name="${id}" </#if> type="button" class="btn btn-${cls}" data-toggle="modal"
	                     data-target="#${target}" ${ext}>
			...
			<div class="modal-content"<#if width!=''> style="width:${width}px;" </#if>>
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
							aria-hidden="true">&${ext};</span></button>
...
<#macro swtich id  size="small" onText="ON" offText="OFF" onColor="primary" offColor="info" col=12 ext="">
	<div class="${autoCol(col)}">
		<div class="form-group">
			...
			<div class="${autoCol(9)}">
				<input type="checkbox" id="${id}" data-size="${size}" data-on-text="${onText}"
				       data-off-text="${offText}" data-on-color="${onColor}" data-off-color="${offColor}" ${ext}>
			</div>
		</div>
	</div>
</#macro>
...
<#macro boxHeader title=''  subTitle='' id='' cls='' ext=''  icon='' >
	<div class="box-header ${cls}" ${f.htmlAttr('id', id)}  ${ext}>
		...
		<#if title!= ''>
			<h2 class="box-title">${title}<#if subTitle!=''>
...
<#macro pagination2 id=''>
	...
	<#--<li ms-class="{'disabled' : @page.pageNo == 1}"><a href="javascript:;" ms-click="@changePage(@page.pageNo-1)"><i class="fa fa-angle-left"></i></a></li>-->
		<li ms-for="el in @page.pageItems" ms-class="{'active' : @page.pageNo == el}" ms-click="@changePage(el)"><a
				href="javascript:;">{{el}}</a></li>
	<#--<li ms-class="{'disabled' : @page.pageNo == @page.pageTotal}"><a href="javascript:;" ms-click="@changePage(@page.pageNo+1)"><i class="fa fa-angle-right"></i></a></li>-->
		<li><a href="javascript:;" ms-click="@changePage(@page.pageTotal)"><i class="fa fa-angle-double-right"></i></a>
		</li>
	</ul>
</#macro>
