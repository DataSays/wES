	...
	<@a.dataGrid>
		<@a.table cls='table-hover' headers=['交易流水号','付款方','收款方','金额','交易类目','创建时间','状态','操作']>
			<tr ms-for="item in @items">
				...
				<td :css="[item.money>=0 && {color:'green'}, item.money<0 && {color:'red'}]">{{item.money|currency}}</td>
				...
				<td>
					<div class="btn-group">
						<@a.actionButton icon='info' ext='ms-on-click="@actionEdit(item)" ms-if="item.Permission == 1"'>支付</@a.actionButton>
						<@a.actionButton icon='info' ext='ms-on-click="@actionEdit(item)" ms-if="item.Permission == 2"'>到账确认</@a.actionButton>
